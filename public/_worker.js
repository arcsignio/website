...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/en/index.html') {
      return Response.redirect(url.origin + '/en/', 301);
    }

    // --- SEO Canonical Redirects ---
    // Redirect old blog URLs from previous site structure to new Astro URLs
    // Old: /blog/en/article.html or /blog/en/article  →  New: /en/blog/article
    const enBlogMatch = url.pathname.match(/^\/blog\/en\/([^/]+?)(?:\.html)?$/);
    if (enBlogMatch && !url.pathname.includes('/images/')) {
      return Response.redirect(url.origin + '/en/blog/' + enBlogMatch[1], 301);
    }
    // Old: /blog/zh-TW/article.html  →  New: /blog/article (Cloudflare serves extensionless)
    const zhBlogMatch = url.pathname.match(/^\/blog\/zh-TW\/([^/]+?)(?:\.html)?$/);
    if (zhBlogMatch && !url.pathname.includes('/images/')) {
      return Response.redirect(url.origin + '/blog/' + zhBlogMatch[1], 301);
    }
