...
| Step | Definition |
| :--- | :--- |
| **Input** | `K` (256-bit private key) |
| **Random** | `S1, S2 ∈ {0,1}²⁵⁶` (CSPRNG) |
| **Compute** | `S3 = K ⊕ S1 ⊕ S2` |
| **Reconstruction** | `K = S1 ⊕ S2 ⊕ S3` ✓ |
...
> **Key insight**
>
> There are 2²⁵⁶ possible private keys. After capturing 2 shards, an attacker still has 2²⁵⁶ equally plausible candidates. **No algorithm can recover the correct key from incomplete shards. That's a mathematical fact, not an engineering compromise.**
...
| Item | Binary | Decimal |
| :--- | :---: | :---: |
| **Private key K** | `1 0 1 0 1 0` | 42 |
| **Shard S₁** | `1 1 0 1 0 1` | 53 (CSPRNG) |
| **Shard S₂** | `0 1 1 0 1 1` | 27 (CSPRNG) |
| **Shard S₃ = K⊕S₁⊕S₂** | `0 0 0 1 0 0` | 4 |

Verification: `53 ⊕ 27 ⊕ 4 = 42` ✓. Could an attacker holding 53 and 27 derive K? No — because S3 is fully random, every value of K corresponds to a valid S3. Scale up to 256 bits and the candidate space becomes 2²⁵⁶, far beyond the brute-force capacity of all the compute on Earth.
...
**1. CSPRNG: shards must really be random**
...
**2. AES-256-GCM second encryption**
...
**3. HKDF: each shard derives its own key**
...
**4. mlock locks reconstruction memory**
...
| --- | --- | --- | --- | --- | --- |
| **XOR Secret Sharing** | Information-theoretic | Very low (plain bitwise XOR) | Personal single-device cold storage | No | 1-5 ms |
| **Shamir Secret Sharing** | Information-theoretic | Medium (GF polynomial interpolation) | Enterprise key backup, SLIP-0039 | No | Low |
| **MPC** | Computational | High (multi-party protocol) | Shared custody vaults | Yes (nodes online) | Higher (network round-trips) |
| **Threshold ECDSA** | Computational | Very high (distributed signing protocol) | Institutional custody, Fireblocks | Yes | High |
...
Both are perfect secret sharing in the information-theoretic sense — an attacker holding fewer than the threshold number of shares learns zero bits about the secret. The difference is attack surface. Shamir requires polynomial interpolation over GF(2²⁵⁶), with more places implementations can go wrong (coefficient overflow, weak randomness, timing leaks). XOR is just bitwise XOR — its core logic fits in fewer than 100 lines and is much easier to audit or formally verify. For single-device cold storage, XOR's smaller attack surface wins.
