...
## ArcSign 如何在簽名前攔截 Drainer

ArcSign 從一開始就把「Drainer 防禦」當成簽名介面的設計核心。具體做法分四層：

            1
            Clear Signing：每筆簽名翻譯成人話

對所有 EIP-712 簽名（包括 Permit、Permit2、ERC-20 approve、ERC-721 `setApprovalForAll`），ArcSign 都做完整 schema 解析並以人話顯示。例如，當收到一個 Permit2 PermitBatch 簽名請求，ArcSign 不會顯示 `0x...`，而是顯示：「你即將授權 [spender 顯示為地址 + 已知標籤]，在 [deadline 翻譯為人類可讀時間]之前，最多移動 [USDC: 5,000 / USDT: 3,000 / DAI: 2,000]」。詳見 [Clear Signing 拆解](/blog/blind-signing-risks)。

            2
            高風險模式自動偵測

ArcSign 內建一個「Drainer 指紋資料庫」，會比對：(a) `spender` 地址是否在已知 Drainer 中繼名單；(b) 簽名型態是否符合常見 Drainer 模板（例如 `Permit2.PermitBatch` 一次抓 5+ 個 token）；(c) 是否為「無限額度 + 極遠 deadline」組合。任一條中標，介面跳全螢幕紅色警告，並要求使用者勾選「我了解這可能是釣魚」才能繼續。

            3
            交易模擬：簽前看見資產淨變化

對 EVM 交易，ArcSign 用本地或受信任 RPC 模擬整筆交易，並在簽名介面顯示「執行後你錢包的變化」：例如「-5,000 USDC, -3,000 USDT, -1 NFT (BAYC #4xxx)」。如果模擬結果跟你的預期不符（例如你只是想 mint 一個新 NFT，但模擬顯示你會損失多個既有 NFT），立刻拒簽。

            4
            私鑰永不離開 USB

即使你不小心被 Drainer 釣到、即使簽下了授權，[ArcSign 的 XOR 三分片金鑰](/blog/xor-encryption-explained)與 [mlock 記憶體保護](/blog/mlock-memory-protection)確保私鑰本體永遠不會被外洩。攻擊面被嚴格限制在「該次授權能影響的資產」 — 而不是「整個錢包未來所有資產」。但這只是最後一道防線，最有效的還是不要簽錯。

            設計哲學：把 [零信任](/blog/zero-trust-wallet) 從金鑰存儲延伸到簽名介面

很多冷錢包把「安全」限縮在「金鑰不外洩」 — 但 Drainer 從來不偷你的金鑰，它只偷你的簽名。ArcSign 把「零信任」的範圍擴展到整個簽名互動流程：每個簽名都被當作「可能是攻擊」來預設處理，要靠 ABI 解析、Drainer 指紋比對、Simulation 三層驗證才放行。
