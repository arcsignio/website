...
從 v1.5.0 起，**餘額與 swap 都不需要 API key**——ArcSign 走公共 RPC + Multicall3 讀取餘額，開箱即用。只有 NFT 與交易歷史才需要免費的 Alchemy（5 鏈）/ NodeReal（BSC）key。詳細步驟請參考[設定教學](/blog/arcsign-beginner-setup-guide)。
...
在 ArcSign 的網路選擇器中選擇 **Polygon**。ArcSign 支援的 7 條主流 EVM 鏈都可以在這裡切換，只要一鍵即可。
...
ArcSign 內建的 DEX Swap 功能支援所有 6 條主流 EVM 鏈，包括 Ethereum、Polygon、BSC（BNB Chain）、Arbitrum、Optimism、Base 等。Swap 由 OpenOcean 和 KyberSwap 聚合器驅動，自動在多個流動性池中搜尋最佳價格。
