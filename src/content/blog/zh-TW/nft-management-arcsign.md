...
支援 Ethereum、Polygon、Arbitrum、Optimism、Base、BSC 與 Avalanche。其中 BSC 使用 NodeReal 的 `nr_getNFTHoldings` 增強 API，Avalanche 透過 Glacier API，其他鏈則透過 Alchemy 的 NFT API 抓取完整中繼資料。

**2. ERC-721 與 ERC-1155 雙標準**
...
ArcSign 同時支援 ERC-721（獨一無二的 NFT）與 ERC-1155（可同時包含多個相同 token 的半同質化標準），並跨 7 條 EVM 鏈顯示：Ethereum、Polygon、Arbitrum、Optimism、Base、BSC 與 Avalanche。其他鏈以 Alchemy API 驅動，BSC 則透過 NodeReal 的 nr_getNFTHoldings 取得完整資料，Avalanche 透過 Glacier API。
