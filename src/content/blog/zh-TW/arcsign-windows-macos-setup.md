...
> **💡 ArcSign 特色**
>
> ArcSign 是免費的 USB [冷錢包](/blog/what-is-cold-storage)，採用 **[XOR 三分片](/blog/xor-encryption-explained)金鑰保護**，確保你的[私鑰](/blog/private-key-management-best-practices)永遠不會離開裝置。支援 7 條主流 EVM 鏈、內建 [DEX Swap](/blog/how-to-dex-swap-arcsign)、[WalletConnect](/blog/walletconnect-dapp-tutorial) v2 簽名，以及 [AES-256](/blog/aes256-encryption-simple) 加密備份。
...
## （選用）設定 Alchemy API 金鑰：NFT 與交易歷史

從 v1.5.0 起，**代幣餘額不需要任何 API 金鑰**——ArcSign 走內建的公共 RPC + Multicall3 讀取全 7 條 EVM 鏈的餘額，價格由 DefiLlama 免費提供，開箱即用。

只有 **[NFT](/blog/nft-management-arcsign) 圖庫與交易歷史**需要 key（那是全鏈索引，公共 RPC 做不到）。需要時設定 API 金鑰非常簡單：
...
打開 ArcSign → 設定 → 提供者（Provider）→ 貼入你的 Alchemy API 金鑰。點擊「儲存」完成設定。ArcSign 現在可以讀取你的 NFT 與交易歷史。（BSC 用 NodeReal key；**Avalanche 走 Glacier，不需要 key**。）

> **💰 為什麼 NFT / 交易歷史需要 Alchemy？**
>
> 餘額免 key，但「列出你持有哪些 NFT」或「拉出完整交易史」本質上是全鏈索引，公共 RPC 做不到，需要 Alchemy 這類索引器。Alchemy 提供快速、可靠的 API 接口，無需你運行自己的節點，免費方案已涵蓋一般使用。私鑰只在 USB 上，Provider 只負責讀取公開鏈上數據。
...
**解決**：將 ArcSign 的安裝資料夾加入防毒軟體的白名單。ArcSign 是免費開源專案（已採 Apache 2.0 全開源），不含任何惡意軟體。
