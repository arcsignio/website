...
ArcSign 對 EIP-712 訊息簽名的處理跟[盲簽](/blog/blind-signing-risks)走同一條 clear-signing 管線 — 一旦識別出是 Permit / Permit2 訊息，就把它當成「授權」而非無害登入來呈現。它實際做的是：

**1. Permit2 解析，不是一堆原始 hex**

簽名介面不會只丟給你 `PermitBatch` + 一堆 typed data hex。ArcSign 會解碼 Permit2 calldata，把關鍵資訊攤出來 — **spender**、**以日期顯示的 expiration**、以及涉及的 token，並對無上限的額度標記。對 EIP-712 的 Permit 簽名也一樣把 spender 與 token 攤出來。所以你看到的不是一串零，而是像「這授權 spender `0x7a2...e91`，額度無上限，到 [日期] 為止」。看不懂的訊息永遠不該被按下。

**2.「授權簽名」風險標記**

Permit / Permit2 的偵測刻意保守 — 用 regex 比對 EIP-712 的 domain / primaryType。一旦命中，或當額度是無上限（max uint）時，這個請求會掛上一個行內的紅色「**授權簽名**」風險徽章，讓你把它當成它等效的鏈上 `approve()` 來看待。

**3. 目的地黑名單檢查（所有人免費）**

ArcSign 在放行簽名前，會把簽名目的地地址比對離線黑名單（OFAC + ScamSniffer + MetaMask 釣魚清單）。如果 spender / 目的地是已知惡意地址，後端會拒簽，除非你明確知情同意。這個檢查離線可用、對所有使用者免費。

**4. 交易模擬（Pro）：簽前預覽資產變化**

ArcSign Pro 能模擬 EVM 交易，在你確認前預覽資產淨變化 — 對「看起來無害、實際在搬錢」的呼叫特別有用。模擬會跳出警告供你判斷（它不會攔截簽名），涵蓋 5 條主要 EVM 鏈，是 Pro 功能。

**5. 私鑰永不離開 USB**

Permit2 攻擊的核心是「**簽錯一張訊息**」 — 所以冷錢包的防禦力，最終取決於你能不能看懂你在簽什麼。ArcSign 的簽名發生在桌面應用程式裡（USB 只是沒有螢幕的加密儲存），[私鑰](/blog/private-key-management-best-practices)以 [XOR 三分片](/blog/xor-encryption-explained)形式存在 USB 上，簽名時只在 [mlock 保護的記憶體](/blog/mlock-memory-protection)中短暫重組。

**設計哲學：每一張 EIP-712 都是潛在的鏈上動作**

ArcSign 不把 EIP-712 訊息當成「比較便宜的簽名」 — 它是與鏈上交易**完全等效的權限授予**。所以 Permit2 訊息會得到與 ERC-20 `approve` 相同的 clear-signing 審視與相同的行內風險標記。看[零信任錢包](/blog/zero-trust-wallet)一文了解 ArcSign 的完整零信任設計。
...
部分能。Ledger 在 2024 Q3 加入了 Permit2 Clear Signing 支援，Trezor 在 2025 Q1 跟上。但**只對 Ethereum 主網上的 Uniswap Permit2 instance** — 其他鏈、其他自定義 Permit2 部署，多數韌體仍不支援，而硬體小螢幕也常常退回原始 hex。ArcSign 是桌面應用程式，簽名介面不受這種限制：它解碼 Permit2 把 spender、以日期顯示的 expiration、token 攤出來，對無上限額度標記，對目的地做離線黑名單檢查（免費），並提供 Pro 模擬。它不宣稱能做 spender 合約靜態分析或跨鏈黑名單比對 — 那些並不是它有的功能。

### Q：用 USB 冷錢包簽 Permit2 還會被釣魚嗎？

「**會**」。USB 冷錢包擋的是「[私鑰外洩](/blog/private-key-management-best-practices)」 — 私鑰永不離開裝置。但 Permit2 釣魚的本質是「**讓你主動簽下一張授權**」，所以冷錢包能做的是「**讓你看清楚你在簽什麼**」這一層。如果使用者還是手動勾選「我了解風險仍要簽」、按下確認，沒有任何冷錢包能擋下。冷錢包的設計目標是把所有資訊以人類可讀的形式攤在你面前，讓你能做出有依據的決定 — 不是奪走你的判斷權。
