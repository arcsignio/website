...
| **支援鏈** | 僅 EVM | 5,500+ 代幣，多鏈 | 多鏈 | 7 條 EVM 鏈：ETH、BSC、Polygon、Arbitrum、Optimism、Base、Avalanche | 僅 EVM |
...
ArcSign 是 USB 冷錢包桌面應用程式（macOS/Windows/Linux）。私鑰以 [AES-256](/blog/aes256-encryption-simple)-GCM 加密分片形式儲存在標準 USB 隨身碟上，從不儲存在連網電腦上。ArcSign 支援七條 EVM 鏈，包含 [WalletConnect](/blog/walletconnect-dapp-tutorial) v2 DApp 互動、內建 [DEX Swap](/blog/how-to-dex-swap-arcsign)，以及業界最便捷的加密備份系統：**.arcsign 文件在匯出時即已完整加密**，無需額外設定密碼。內建[代幣授權](/blog/token-approval-revoke)管理。**ArcSign 免費使用，完全開源（Apache 2.0），以零成本提供冷錢包等級安全性。**
...
- **從第一天起支援多鏈**——以太坊、BSC、Polygon、Arbitrum、Optimism、Base 和 Avalanche 共 7 條 EVM 鏈，全部從單一錢包和 USB 管理。
...
ArcSign 非常適合新手：免費、支援 macOS/Windows/Linux、支援 7 條 EVM 鏈（ETH/BSC/Polygon/Arbitrum/Optimism/Base/Avalanche），提供引導式設置精靈。.arcsign 加密備份消除了紙本助記詞管理的負擔。對於只使用單一鏈的完全新手，MetaMask 也是有效起點——但要了解它是熱錢包。

### Q：ArcSign 支援比特幣嗎？

目前不支援。ArcSign 專注於 EVM 生態，支援 7 條主流 EVM 鏈：以太坊（ETH）、BSC、Polygon、Arbitrum、Optimism、Base、Avalanche——全部由同一個 USB 上的單一錢包設置管理。如果你的資產主要在這些 EVM 鏈上，ArcSign 以零成本提供冷錢包安全性；若你需要原生 Bitcoin 冷儲存，目前需搭配專門支援 BTC 的工具。
