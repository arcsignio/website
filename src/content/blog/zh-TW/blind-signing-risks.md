...
EIP-712 本身是好設計 — 它的目的就是讓簽名變得**更清楚**，而不是更模糊。問題在於：**錢包與硬體裝置必須真的解析它**。如果硬體錢包只是把 EIP-712 也當 Hex 顯示，那再好的標準也救不了使用者。ArcSign 的 clear-signing 會解析 EIP-712 的 typed data，把欄位攤成可讀的列 — spender 是誰、額度多少、deadline 多久。
...
ArcSign 從第一版設計就把「不讓使用者盲簽」列為核心原則。它實際做的是：

**1. 本地 clear-signing：解析常見的高風險呼叫**

ArcSign 在本地把 WalletConnect / mint 的 calldata 與 EIP-712 typed data 解碼成可讀的意圖。它不是靠一個龐大的預載資料庫，而是用一組精選的 ABI 片段、以 4-byte selector 比對 — 涵蓋真正會帶風險的型態：ERC-20 / ERC-721（`transfer`、`approve`、`setApprovalForAll`）、Permit2，以及主流 DEX router 的 swap。所以你看到的不是 `0x095ea7b3...`，而是像 `approve(Uniswap V3 Router, 100 USDC)` 這樣的人話。遇到本地解不出來的合約，ArcSign 提供選用的線上 **Sourcify** ABI 後援；若仍解不出來，它會老實退回原始 hex 並警告你，要求勾選「我了解風險仍要簽」才能繼續。

**2. EIP-712 以可讀欄位呈現 + Permit/Permit2 風險標記**

對 EIP-712 結構化簽名，ArcSign 把欄位攤成可讀的列（spender 是誰、額度多少、deadline 多久），而不是一堆 hex。Permit / Permit2 的偵測刻意保守 — 用 regex 比對 EIP-712 的 domain / primaryType — 一旦命中，這張簽名會被標上一個行內的「**授權簽名**」風險徽章，讓你把它當成它本質上的授權行為來看待，而不是無害的登入。

**3. 無限額度與 setApprovalForAll：行內紅旗**

當 `amount = 2^256-1`（無限）或 `setApprovalForAll(operator, true)` 出現時，ArcSign 會在該欄位掛上一個**行內紅旗 / 徽章**，讓 Drainer 簽名裡最危險的這兩種型態不會混在請求中被忽略。

**4. 交易模擬 + 資產淨變化預覽（Pro）**

對 EVM 交易，ArcSign Pro 會在簽名前模擬整筆交易，並顯示「執行後你錢包的變化」：例如「-1.5 ETH、+3,124 USDC、-1 NFT（CryptoPunk #4xxx）」。如果模擬結果出現「-N NFT」但你以為自己只是在「mint 一個 NFT」，停下來、拒簽。模擬會跳出警告供你判斷（它不會攔截簽名），涵蓋 5 條主要 EVM 鏈，是 Pro 功能。
...
不一定。Ledger 與 Trezor 在 2023 之後陸續支援部分 Clear Signing（特別是 Uniswap、1inch 等主流 DApp），但**對冷門合約、新部署的合約、自訂 EIP-712 結構，硬體螢幕仍然只能顯示 Hex 或函數簽名**。研究機構 a16z crypto 在 2024 的測試中發現，主流硬體錢包對「隨機 100 個 DApp 的簽名請求」的 Clear Signing 覆蓋率約 60–75%，剩下 25–40% 仍是盲簽。ArcSign 是桌面應用程式，簽名介面不受硬體小螢幕限制：對常見的高風險型態做 clear-signing，遇到不認識的合約能從 Sourcify 抓 ABI，真的解不出來時退回明確的「我了解風險仍要簽」確認步驟。
...
兩步驟：(1) 先嘗試選用的線上 **Sourcify** ABI 查詢來解析這筆呼叫；(2) 抓不到的話，會退回原始 hex 並警告你正在盲簽，要求你勾選一個明確的確認框「我了解風險仍要簽」才能繼續。這個刻意的「停一下」 — 拒絕讓一筆無法解讀的簽名顯得安全 — 正是重點所在：看不懂，最誠實的預設就是停下來。
