...
description: "錢包看不到代幣怎麼辦？本文系統化拆解 ERC-20 標準、為什麼需要手動加代幣，並示範 ArcSign 上「新增 / 隱藏 / 自訂代幣」的完整流程，附假代幣詐騙辨識、跨 7 條 EVM 鏈的代幣治理 SOP 與隱私小技巧。免費功能，冷錢包離線簽名。"
pubDate: 2026-04-27
locale: zh-TW
tags: ["產品教學"]
...
## 六、跨 7 條 EVM 鏈的代幣治理 SOP

ArcSign 同時支援 **Ethereum / BSC / Polygon / Arbitrum / Optimism / Base / Avalanche**，每條鏈都有各自的 token list。如果你是多鏈用戶，建議建立一份「個人代幣治理 SOP」，每月固定執行一次：
...
從 v1.5.0 起，餘額查詢走內建的公共 RPC + Multicall3，**不需要任何 API key**，查詢也不會把你的地址綁到某個第三方帳號上。如果你對預設的公共端點有更高的隱私要求，可在 ArcSign 設定中切換到自訂 RPC（例如 Infura、QuickNode、或自架節點）——自架或分流可進一步降低 IP 關聯風險。詳情可參考 [自訂 RPC 節點教學](/blog/arcsign-custom-rpc)（即將上線）。
...
錢包預設的代幣清單只涵蓋幾十到幾百種主流代幣（如 USDT、USDC、WBTC 等）。一條 EVM 鏈上實際存在的 ERC-20 代幣超過數百萬種，不可能全部預載。如果你轉入的是非主流代幣，鏈上其實已經到帳，只是錢包介面還沒「認識」它。手動透過合約地址新增之後，餘額就會立即顯示。ArcSign 的「新增自訂代幣」功能可在 7 條 EVM 鏈上一鍵搞定。
...
新增 / 隱藏 / 自訂代幣本身只是修改 USB 內的本地 token list，不需要任何鏈上交易、不需要簽名、私鑰完全不會被觸碰。讀取餘額時走公共 RPC + Multicall3 的唯讀查詢（免 key），只送出公開的錢包地址。所有寫入鏈上的動作（例如 transfer、swap、revoke）才會用到私鑰，並且永遠在 USB 內透過 [XOR 三分片](/blog/xor-encryption-explained)離線重組、簽名、立即清零，曝露窗口僅 1-5 毫秒。

### Q：ArcSign 代幣管理跟 MetaMask、Trust Wallet 的差別在哪裡？

三者都允許自訂代幣，但安全模型不同：MetaMask / Trust Wallet 是熱錢包，私鑰存在你常上網的裝置上，任何代幣管理只要 PC 被入侵都可能被連根拔起。ArcSign 是 USB [冷錢包](/blog/what-is-cold-storage)，代幣清單與私鑰都儲存在 USB 內、無網際網路時也能離線管理；簽名一律離線完成，私鑰永不離開裝置。此外 ArcSign 內建 7 條 EVM 鏈（含 BSC）的官方代幣清單，並對假合約自動標註警告 — 這是熱錢包通常缺乏的安全層。
