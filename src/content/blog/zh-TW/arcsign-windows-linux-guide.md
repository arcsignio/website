...
ArcSign 是**免費軟體**（未來用戶破萬後將開源），不收取任何訂閱費。`.arcsign` 備份檔**匯出即為 [AES-256](/blog/aes256-encryption-simple) 加密**，無需額外設定密碼步驟。[私鑰](/blog/private-key-management-best-practices)從不以完整形式存在於任何單一位置，簽名時的曝露窗口控制在 **1–5 毫秒**。
...
## 首次啟動：Alchemy API 金鑰與 Provider 設定

ArcSign 需要連線到區塊鏈節點才能顯示餘額與送交易。為了**隱私與速度**，我們不使用公共 RPC，而是請你使用自己的免費 API Key。Alchemy 的免費方案每月可處理 3 億個計算單元，對個人用戶綽綽有餘。

            1
            註冊 Alchemy 免費帳號

前往 alchemy.com → Sign up → 選「Individual」。建立一個新 App，名稱隨意，選擇「Ethereum Mainnet」。完成後到 Dashboard 的 API Keys 區，複製 **API Key**（不是 HTTP URL）。

            2
            在 ArcSign 貼上金鑰

打開 ArcSign → 設定（右下齒輪圖示）→ Providers → Alchemy Key，貼上剛才複製的金鑰。ArcSign 會自動用這把金鑰連線 Ethereum、Polygon、Arbitrum、Optimism、Base 等 EVM 鏈。BSC 使用 NodeReal 提供的公開端點，不需要額外設定。

            3
            建立或匯入錢包

接下來就可以插入你的 USB 隨身碟，選擇「建立新錢包」（會生成 12 字[助記詞](/blog/seed-phrase-backup-guide)）或「匯入 .arcsign 備份檔」。新手完整設定流程請見 [10 分鐘新手入門教學](/blog/arcsign-beginner-setup-guide)。
...
| 餘額顯示為 0 或載入失敗 | Alchemy API Key 未設定或達用量上限 | 檢查 Settings → Providers；Alchemy Dashboard 查看用量 |
...
### Q：為什麼需要 Alchemy API Key？ArcSign 不能直接連區塊鏈嗎？

技術上可以用公共 RPC，但公共節點有兩個問題：(1) 你的查詢 IP 會被節點提供者記錄，可能洩漏錢包活動；(2) 公共節點速率限制嚴格，常常超時。使用你自己的 Alchemy 免費 Key 能同時提升隱私與速度。ArcSign 只會在本機使用這把 Key，不會回傳給我們。更多技術細節見 [自訂 RPC 教學](/blog/arcsign-custom-rpc)。
