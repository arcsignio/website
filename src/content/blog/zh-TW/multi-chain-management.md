...
description: "完整教學：如何用 ArcSign 同時管理 BTC、ETH、BSC 等 7 條鏈的加密資產。從新增多鏈錢包、設定 RPC 節點，到跨鏈轉帳和 DEX Swap，一站式搞定所有多鏈操作的完整圖解流程教學。，附各鏈 RPC 節點設定建議、Gas 費用管理最佳實踐完整整理。"
pubDate: 2026-03-20
locale: zh-TW
tags: ["產品教學"]
...
**一支 USB、一組[助記詞](/blog/seed-phrase-backup-guide)、7 條鏈**。ArcSign 透過 [HD 錢包](/blog/mpc-vs-hd-wallet)標準（[BIP-39](/blog/key-derivation-bip39-44)/[BIP-44](/blog/key-derivation-bip39-44)），讓你在同一個[冷錢包](/blog/what-is-cold-storage)介面中管理 Bitcoin 和所有主流 EVM 鏈的資產，[私鑰](/blog/private-key-management-best-practices)全部安全地存在 USB 裝置上。

## ArcSign 支援的 7 條鏈一覽

ArcSign 目前支援 Bitcoin 以及 6 條主流 EVM 相容鏈。每條鏈的餘額、代幣、[NFT](/blog/nft-management-arcsign) 和交易紀錄都可以在 Dashboard 中直接查看和操作。
...
ArcSign 支援 7 條鏈：Bitcoin（BTC）以及 6 條主流 EVM 相容鏈，包括 Ethereum、BSC、Polygon、Arbitrum、Optimism、Avalanche、Base、Fantom 等。所有鏈共用同一組助記詞，透過 BIP-44 衍生出各鏈的獨立地址。
