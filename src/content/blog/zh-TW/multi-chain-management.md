---
title: "BTC + ETH + BSC 一站管理：ArcSign 跨鏈教學"
description: "完整教學：如何用 ArcSign 同時管理 BTC、ETH、BSC 等 22 條鏈的加密資產。從新增多鏈錢包、設定 RPC 節點，到跨鏈轉帳和 DEX Swap，一站式搞定所有多鏈操作的完整圖解流程教學。，附各鏈 RPC 節點設定建議、Gas 費用管理最佳實踐完整整理。"
pubDate: 2026-03-20
locale: zh-TW
tags: ["產品教學"]
...
**一支 USB、一組[助記詞](/blog/seed-phrase-backup-guide)、22 條鏈**。ArcSign 透過 [HD 錢包](/blog/mpc-vs-hd-wallet)標準（[BIP-39](/blog/key-derivation-bip39-44)/[BIP-44](/blog/key-derivation-bip39-44)），讓你在同一個[冷錢包](/blog/what-is-cold-storage)介面中管理 Bitcoin 和所有主流 EVM 鏈的資產，[私鑰](/blog/private-key-management-best-practices)全部安全地存在 USB 裝置上。

## ArcSign 支援的 22 條鏈一覽

ArcSign 目前支援 Bitcoin 以及 21 條 EVM 相容鏈。每條鏈的餘額、代幣、[NFT](/blog/nft-management-arcsign) 和交易紀錄都可以在 Dashboard 中直接查看和操作。
...
BIP-44 定義了一個結構化的路徑格式：`m/purpose'/coin_type'/account'/change/address_index`。其中 **coin_type** 這個欄位就是區分不同鏈的關鍵。Bitcoin 的 coin type 是 0，Ethereum 及所有 EVM 鏈共用 60。因此，同一組助記詞會衍生出完全不同的 BTC 地址和 ETH 地址，保持各鏈資產完全隔離。
...
主金鑰根據 BIP-44 路徑，為每條鏈衍生出獨立的子金鑰。BTC 走 `m/44'/0'/0'`，ETH 和 EVM 鏈走 `m/44'/60'/0'`。每條鏈的私鑰數學上不可逆推回主金鑰。
...
為了讀取鏈上數據（餘額、交易紀錄、代幣資訊），ArcSign 需要連接 Provider。大部分 EVM 鏈使用 Alchemy（免費方案即可），BSC 使用 NodeReal。在設定頁面輸入你的 API Key 即可。
...
在 ArcSign 中發送資產非常直覺：選擇你要操作的鏈、輸入接收地址和金額，確認後 ArcSign 會在 USB 上用該鏈的私鑰進行簽名。整個簽名過程在本地完成，私鑰不會離開你的裝置。BTC 轉帳走原生 UTXO 協議，EVM 鏈的轉帳則支援 EIP-1559 的動態 Gas Fee 機制。
...
雖然 EVM 鏈的地址格式相同（都是 0x 開頭），但不同鏈的資產不互通。將 ETH 發送到 BSC 地址（或反過來）可能導致資產無法取回。ArcSign 在發送頁面會清楚標示目前選擇的鏈，幫你避免這類錯誤。BTC 的地址格式完全不同（bc1 開頭），更不可能混淆。
...
ArcSign 支援 22 條鏈：Bitcoin（BTC）以及 21 條 EVM 相容鏈，包括 Ethereum、BSC、Polygon、Arbitrum、Optimism、Avalanche、Base、Fantom 等。所有鏈共用同一組助記詞，透過 BIP-44 衍生出各鏈的獨立地址。
